...
# 전역변수
memory = []
head, current, pre = None, None, None # pre 새로 만든 값 이전의 값
...
def deleteNode(deleteData):
    ...
    while current.link != None: # 첫번째 이외 노드 삭제
        ...
        if current.data == deleteData:
            pre.link = current.link # current 가리키는 노드를 pre 가 가리키도록(삭제)
            del(current)
            return
